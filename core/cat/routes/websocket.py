...
async def receive_message(websocket: WebSocket, stray: StrayCat):
    ...
    while True:
        ...
        portfolio_token = get_portfolio_token(websocket.headers)
        stray.portfolio_token = portfolio_token
...
def get_portfolio_token(headers):
    if headers is None or headers.get('cookie', None) is None:
        return None

    headers_list = headers.get('cookie').split(';')
    res_dict = {}
    for header in headers_list:
        key_value = header.split('=')
        res_dict[key_value[0].strip()] = key_value[1]
